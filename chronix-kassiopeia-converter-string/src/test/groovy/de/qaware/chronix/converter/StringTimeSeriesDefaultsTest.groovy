...
class StringTimeSeriesDefaultsTest extends Specification {
    ...
    def "test default group by"() {
        ...
        when:
        def groupBy = StringTimeSeriesDefaults.GROUP_BY.apply(straceTimeSeries)
    ...
    def "test default reduce"() {
        ...
        def ts2 = new StringTimeSeries.Builder("metric")
                ...
                .build();
        when:
        def merged = StringTimeSeriesDefaults.REDUCE.apply(ts1, ts2)
    ...
    def "test private constructor"() {
        when:
        StringTimeSeriesDefaults.newInstance()
        then:
        noExceptionThrown()
    }
}
