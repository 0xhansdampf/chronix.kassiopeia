...
import de.qaware.chronix.converter.serializer.JsonKassiopeiaStringSerializer;
import de.qaware.chronix.converter.serializer.ProtoBufKassiopeiaStringSerializer;
 ...
 */
public class StringTimeSeriesConverter implements TimeSeriesConverter<StringTimeSeries> {
    ...
    @Override
    public StringTimeSeries from(BinaryTimeSeries binaryTimeSeries, long queryStart, long queryEnd) {
        ...
        StringTimeSeries.Builder builder = new StringTimeSeries.Builder(metric);
    ...
    private void fromProtocolBuffers(BinaryTimeSeries binaryTimeSeries, long queryStart, long queryEnd, StringTimeSeries.Builder builder) {
        ProtoBufKassiopeiaStringSerializer.from(binaryTimeSeries.getPoints(), binaryTimeSeries.getStart(), binaryTimeSeries.getEnd(), queryStart, queryEnd, builder);
    }

    private void fromJson(BinaryTimeSeries binaryTimeSeries, long queryStart, long queryEnd, StringTimeSeries.Builder builder) {
        String jsonString = binaryTimeSeries.get(DATA_AS_JSON_FIELD).toString();
        //Second deserialize
        JsonKassiopeiaStringSerializer serializer = new JsonKassiopeiaStringSerializer();
        serializer.fromJson(jsonString.getBytes(Charset.forName(JsonKassiopeiaStringSerializer.UTF_8)), queryStart, queryEnd, builder);
    }
    ...
    @Override
    public BinaryTimeSeries to(StringTimeSeries timeSeries) {
        LOGGER.debug("Converting {} to BinaryTimeSeries", timeSeries);
        BinaryTimeSeries.Builder builder = new BinaryTimeSeries.Builder();

        //serialize
        byte[] compressedPoints = ProtoBufKassiopeiaStringSerializer.to(timeSeries.points().iterator());
